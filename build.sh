...
${nuget_cmd} install src/IO.Pesamoni_csharp/packages.config -o packages;
...
echo "[INFO] Run 'mcs' to build bin/IO.Pesamoni_csharp.dll"
mcs -langversion:${langversion} -sdk:${sdk} -r:bin/Newtonsoft.Json.dll,bin/JsonSubTypes.dll,\
bin/RestSharp.dll,\
System.ComponentModel.DataAnnotations.dll,\
System.Runtime.Serialization.dll \
-target:library \
-out:bin/IO.Pesamoni_csharp.dll \
-recurse:'src/IO.Pesamoni_csharp/*.cs' \
-doc:bin/IO.Pesamoni_csharp.xml \
-platform:anycpu
...
then
  ...
  echo "[INFO] bin/IO.Pesamoni_csharp.dll was created successfully"
fi
